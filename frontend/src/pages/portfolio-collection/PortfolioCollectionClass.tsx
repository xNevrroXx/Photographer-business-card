...
const RedirectToCollection: FC<IPropsRedirect> = ({collectionsPhotoProp, urlJson}: IPropsRedirect) => {
    ...
    return (
        <>
            <PortfolioCollectionClass collectionsPhoto={collectionsPhotoProp} urlJson={urlJson} nameCollection={nameCollection!}/>
        </>
    )
}


class PortfolioCollectionClass extends Component<IPropsComponent, IStateComponent>{
    ...
    componentDidMount() {
        if(this.props.collectionsPhoto && this.props.collectionsPhoto.length == 0)
        {
